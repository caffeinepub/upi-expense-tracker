...
import { Calendar, Receipt, Tag, TrendingDown } from "lucide-react";
...
export default function Dashboard() {
  ...
  const { data: monthlySummary = [], isLoading: summaryLoading } =
    useGetMonthlyExpenseSummary(selectedYear, selectedMonth);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Track and analyze your UPI spending
          </p>
        </div>
        <MonthSelector
          year={selectedYear}
            ...
            {txLoading ? (
              <div className="space-y-3">
                {[1, 2, 3, 4, 5].map((i) => (
                  <Skeleton
                    key={i}
                    className="h-14 rounded-xl bg-surface-elevated"
                  />
                ))}
              </div>
            ) : (
