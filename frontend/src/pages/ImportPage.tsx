    ...
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-8">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Upload your UPI transaction export to automatically categorize and track your expenses.
        </p>
      </div>
      ...
      <div className="flex items-start gap-3 p-4 rounded-xl bg-mint/10 border border-mint/30">
        ...
        <div>
          <p className="text-sm font-semibold text-foreground">Your data stays private</p>
          <p className="text-sm text-muted-foreground mt-0.5">
            All file parsing happens entirely in your browser. No raw file content is ever sent to any server.
            Only the structured transaction data you choose to import is stored on-chain under your identity.
          </p>
        </div>
      </div>
      ...
      <div className="bg-surface rounded-2xl p-6 border border-border shadow-card space-y-3">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h2 className="text-base font-semibold text-foreground">Supported File Formats</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            { ext: 'CSV', desc: 'Comma-separated values export from any UPI app' },
            { ext: 'TXT', desc: 'Plain text transaction history files' },
            { ext: 'PDF', desc: 'PDF statement exports from GPay, PhonePe, and Paytm' },
          ].map(({ ext, desc }) => (
            <div key={ext} className="flex flex-col gap-1 p-3 rounded-xl bg-background border border-border">
              <span className="text-xs font-bold text-mint tracking-widest">.{ext}</span>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold text-foreground">How to Export from Your UPI App</h2>
        </div>
